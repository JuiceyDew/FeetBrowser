...
# and nowhere else, which is what these check, against a real server.
#
# FEETBROWSER_SCALE stands in for an Xft.dpi of 192. A CI runner's Xvfb has
# no resource database at all and so reports nothing, which is the 1x case
# every other test in this file already covers; the override is how the 2x
# path gets exercised anywhere.
...
def live_a_dense_display_gets_a_window_of_device_pixels():
    """The server's opinion again: the window really is twice the size, and
    the browser above it still believes it is 320x240."""
...
def live_a_dense_frame_lands_one_pixel_for_one():
    """Read back off the server, which is the only way to tell a frame that
    was blitted one to one from one the display stretched.

    A rectangle 10 CSS pixels across has to reach device pixel 19 and stop
    before device pixel 21. Before the buffer was allocated in device pixels
    it covered ten device pixels and the server scaled nothing, so pixel 19
    was background -- which is what the whole page looked like.
    """
    with _DenseSession(320, 240) as win:
